...
from .models import HLPPrivatecKey, HLP_otherPrivatecKey, PaillierPrivatecKey, SealBFVPrivatecKey, SealCKKSPrivatecKey, \
    Cateory, Content, GetNum, Wage, UserIC
...
def renew_Paillier_privateKey(request):
    if request.method == 'POST':  # 当提交表单时
        ...
        if request.POST:
            ...
            prk = PaillierPrivatecKey.objects.get(id=1)
            prk.g = g
            prk.lamda = lamda
            prk.n = n
            prk.u = u
...
def renew_HLP_privateKey(request):
    if request.method == 'POST':  # 当提交表单时
        ...
        if request.POST:
            ...
            prk = HLPPrivatecKey.objects.get(id=1)
            prk.Deta = Deta
            prk.A = A
            prk.B = B
            prk.N = N
...
def renew_HLP_other_privateKey(request):
    if request.method == 'POST':  # 当提交表单时
        ...
        if request.POST:
            ...
            prk = HLP_otherPrivatecKey.objects.get(id=1)
            prk.Deta = Deta
            prk.A = A
            prk.B = B
            prk.N = N
...
def renew_BFV_privateKey(request):
    if request.method == 'POST':  # 当提交表单时
        # 判断是否传参
        if request.POST:
            secret_key = request.POST.get('secret_key')
            # prk = SealBFVPrivatecKey(secret_key=secret_key)
            prk = SealBFVPrivatecKey.objects.get(id=1)
            prk.secret_key = secret_key
            prk.save()
            return JsonResponse({
                "status_code": 0,
                "data": "renew success!",
...
def renew_CKKS_privateKey(request):
    if request.method == 'POST':  # 当提交表单时
        # 判断是否传参
        if request.POST:
            secret_key = request.POST.get('secret_key')
            # prk = SealCKKSPrivatecKey(secret_key=secret_key)
            prk = SealCKKSPrivatecKey.objects.get(id=1)
            prk.secret_key = secret_key
            prk.save()
            return JsonResponse({
                "status_code": 0,
...
def temporary(request):
    if request.method == "GET":
        ...
        if user_ic.IC_num is None or user_ic.IC_num == '':
            return JsonResponse({
                "status_code": 1,
                "data": "未认证"
            })
        else:
            url = "http://127.0.0.1:8020/opens/temporary"
            data = int(json.loads(
                requests.post(url, data={'IC_num': user_ic.IC_num}).text)['data'])
            return JsonResponse({
                "status_code": 0,
                "data": data
            })
...
def getQuota(request):
    if request.method == "GET":
        ...
        if user_ic.IC_num is None or user_ic.IC_num == '':
            ...
            url = "http://127.0.0.1:8020/opens/getQuota?IC_num=" + user_ic.IC_num
            ...
            if result['enalg'] == 1:
                ...
                data = paillier.paillier_decryption(result['data'], int(prk.g), int(prk.lamda), int(prk.n),
                                                    int(prk.u))
            elif result['enalg'] == 2:
                prk = HLPPrivatecKey.objects.last()
                data = HLP.two_to_long(
                    HLP.lattice_decryption(result['data'], eval(str(prk.Deta)), eval(str(prk.A)), eval(str(prk.B)),
                                           prk.N, prk.mods, prk.q), prk.N)
            elif result['enalg'] == 3:
                prk = HLP_otherPrivatecKey.objects.last()
                data = HLP_other.two_to_long(
                    HLP_other.lattice_decryption(result['data'], eval(str(prk.Deta)), eval(str(prk.A)),
                                                 eval(str(prk.B)), prk.N, prk.mods, prk.q), prk.N)
            elif result['enalg'] == 4:
                prk = SealBFVPrivatecKey.objects.last()
                url = 'http://39.102.39.63:9000/seal/BFV_Decrypt'
                data = int(json.loads(
                    requests.post(url, data={'x_encrypted': result['data'], 'secret_key': prk.secret_key}).text)[
                               'x_decrypted'])
            elif result['enalg'] == 5:
                prk = SealBFVPrivatecKey.objects.last()
                ...
                data = int(json.loads(
                    requests.post(url, data={'x_encrypted': result['data'], 'secret_key': prk.secret_key}).text)[
                               'x_decrypted'])
            data = int(data/int(result['LDivisor']))
        return JsonResponse({
            "status_code": 0,
            "data": data
        })
        ...
        if get_wage is not None and get_wage == "true" and user_id is not None:
            ...
            if user_ic.IC_num is None or user_ic.IC_num == '':
                ...
                if get_date is not None:
                    url = "http://127.0.0.1:8020/opens/get_info?wage=true&IC_num=" + user_ic.IC_num + "&date=" + get_date
                else:
                    url = "http://127.0.0.1:8020/opens/get_info?wage=true&IC_num=" + user_ic.IC_num
                data = requests.get(url)
                # result = json.loads(data.text)
                result = data.json()
                if result['enalg'] == 1:
                    ...
                    for data in result['data']:
                        data['pf'] = paillier.paillier_decryption(data['pf'], int(prk.g), int(prk.lamda), int(prk.n),
                                                                  int(prk.u)) / 100
                        data['ss'] = paillier.paillier_decryption(data['ss'], int(prk.g), int(prk.lamda), int(prk.n),
                                                                  int(prk.u)) / 100
                elif result['enalg'] == 2:
                    prk = HLPPrivatecKey.objects.last()
                    for data in result['data']:
                    ...
                    for data in result['data']:
                        data['pf'] = HLP_other.two_to_long(
                            HLP_other.lattice_decryption(data['pf'], eval(str(prk.Deta)), eval(str(prk.A)),
                                                         eval(str(prk.B)),
                                                         prk.N, prk.mods, prk.q), prk.N) / 100
                        data['ss'] = HLP_other.two_to_long(
                            HLP_other.lattice_decryption(data['ss'], eval(str(prk.Deta)), eval(str(prk.A)),
                                                         eval(str(prk.B)),
                                                         prk.N, prk.mods, prk.q), prk.N) / 100
                elif result['enalg'] == 4:
                    prk = SealBFVPrivatecKey.objects.last()
                    url = 'http://39.102.39.63:9000/seal/BFV_Decrypt'
                    # print(result['data'][0]['pf'])
                    # print("+++++++")
                    # print(json.loads(requests.post(url, data={'x_encrypted': result['data'][0]['pf'], 'secret_key': prk.secret_key}).text)['x_decrypted'])
                    # print("++++++++++++")
                    # print(requests.post(url, data={'x_encrypted': result['data'][0]['pf'], 'secret_key': prk.secret_key}).text)
                    for data in result['data']:
                        data['pf'] = int(json.loads(
                            requests.post(url, data={'x_encrypted': data['pf'], 'secret_key': prk.secret_key}).text)[
                                             'x_decrypted']) / 100
                        data['ss'] = int(json.loads(
                            requests.post(url, data={'x_encrypted': data['ss'], 'secret_key': prk.secret_key}).text)[
                                             'x_decrypted']) / 100
                elif result['enalg'] == 5:
                    prk = SealBFVPrivatecKey.objects.last()
                    url = 'http://39.102.39.63:9000/seal/CKKS_Decrypt'
