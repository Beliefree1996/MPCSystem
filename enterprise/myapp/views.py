...
from . import RiskModelFunction as RMF


# Create your views here.
def quick_add(a, b):  # Paillier 8A+5B->quick_add(Enc(A),8)+quick_add(Enc(B),5)
    ans = 0
    while b != 0:
        if b & 1:
            ans = ans.add(a)
...
def HomAdd(*args):  # 同态加法，第一个参数是flag，1是Paillier，2是HLP，3是HLP_other
    if len(args) == 3:
        add = args[1] + args[2]
    elif len(args) == 5:
        if args[0] == 2:
            add = HLP.MatrixAdd(args[1], args[2], 1, 2 * args[3], args[4])
        elif args[0] == 3:
            add = HLP_other.MatrixAdd(args[1], args[2], 1, 2 * args[3], args[4])
    return add


def getQuota_linear(request):
    if request.method == 'POST':  # 当提交表单时
        ...
        if request.POST:
            ...
            # print("++++++++++++")
            if operation == 1:  # paillier
                ss = int(request.POST.get('ss'))
                pf = int(request.POST.get('pf'))
